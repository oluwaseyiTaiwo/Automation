...
def main():
    ...
    for x in parsed_interface:
        if x['phy'] != "*down" and x['interface'].startswith(("GigabitEthernet", "Eth-Trunk")):
            ...
            elif "VLAN ID" in config_qos:
                count_vlan_error += 1
                node_ip_error.append(f'{ip_address}')
                port_error.append(f'{interface_name}')
                node1_name_error.append(f'{node_name}')
                status_1.append('VLAN-Error')
    ...
    total = count_error + count_done + count_vlan_error + count_trunk_error
    # print(total)
    status.append('Done')
    ...
    count_vlan_error_list.append(f'{count_vlan_error}')
    total_done.append(f'{total}')
...
with open(f'QOS_Huawei_Display_ Date({now_date1_NF})__ Time({now_time1_NF}).txt', "a") as display:
            ...
            try:
                try:
                    ...
                    connection = netmiko.ConnectHandler(
                        ip=ip_address, device_type="huawei", username="xxx", password="xxxx", global_delay_factor=0.1, timeout=10)
                ...
                try:
                    ...
                    print(nodenum, "Huawei Connecting with TELNET to",
                          node_name, "\n", file=display)
                    connection = netmiko.ConnectHandler(
                        ip=ip_address, device_type="huawei_telnet", username="xxxxxxx", password="xxxx", global_delay_factor=0.1, timeout=10)
                    ...
                    print(nodenum, "Datacom Connecting with TELNET to",
                          node_name, "\n", file=display)
                    connection = netmiko.ConnectHandler(
                        ip=ip_address, device_type="huawei_telnet", username="xxxxxx", password="xxxx", global_delay_factor=0.1, timeout=10)
    ...
    df_out = pd.DataFrame(list(zip(node_ip, node1_name, status, count_done_list, count_vlan_error_list, count_trunk_error_list, count_error_list, total_done)), columns=[
                          'IP Address', 'Router Name', 'Status', "Done", "Vlan-Error", "Trunk-Error", "Other-Error", "Total-Port"])
    df_out.to_excel(
        f'QOS_OUTPUT_Huawei_Date_({now_time})__ Time_({now_time_NF}).xlsx', index=False)
    ...
    df_out_1.to_excel(
        f'QOS_OUTPUT_Huawei_Port_ERROR_Date_({now_time})__ Time_({now_time_NF}).xlsx', index=False)

    # CREATING AND NAMING FILE
    print(
        f"File QOS_OUTPUT_Huawei_Date_({now_time})__ Time_({now_time_NF}).xlsx created \n Find it to see output")
    print(
        f"File QOS_OUTPUT_Huawei_Date_({now_time})__ Time_({now_time_NF}).xlsx created \n Find it to see output", "\n", file=display)
    print(
        f"File QOS_OUTPUT_Huawei_Port_ERROR_Date_({now_time})__ Time_({now_time_NF}).xlsx created \n Find it to see output")
    print(
        f"File QOS_OUTPUT_Huawei_Port_ERROR_Date_({now_time})__ Time_({now_time_NF}).xlsx created \n Find it to see output", "\n", file=display)
